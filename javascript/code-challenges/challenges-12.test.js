...
const replaceZeros = (string) => {
  var res = string.replace(/0/g, 'zero');
  return res;
};
...
const validateEmail = (email) => {
  return /^\w+.\w+@([\w]+\.){1}(com|net|org)$/.test(email);
  // Solution code here...
}
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6
...
const validatePhoneNumber =(phoneNumber) => /^(\((\d{3})\)[\s]?|(\d{3})[\s-]?)(\d{3})[\s-]?(\d{4})$/.test(phoneNumber);
  // Solution code here...
