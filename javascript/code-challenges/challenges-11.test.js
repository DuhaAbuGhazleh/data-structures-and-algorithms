...
function transformToLis(obj){
  let result = Object.keys(obj).map((key) => `<li>${key}: ${obj[key]}</li>`);
  return result
};
...
const count = (target, input) => input.flat().reduce((a, v) => (v === target ? a + 1 : a), 0);
  // Solution code here...
...
const totalSum = (input) => input.flat().reduce(((a, v) => a + v), 0);
...
const divisibleByFiveTwoToThePower = (input) => {
  let result = input.map((element) => element.filter((value) => value % 5 === 0 && typeof value === 'number')
...
let findMaleAndFemale = (data) => {
  let result = data.filter((value) => value.gender === 'female' || value.gender === 'male'
  ).map((value) => value.name).join(' and ');
  return result;
};
...
let findShortest = (data) => {
  let result = data.reduce((acc, cur) => Number(acc.height) < Number(cur.height) ? acc : cur, 0);
  return result.name;
};
